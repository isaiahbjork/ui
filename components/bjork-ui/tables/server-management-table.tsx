...
export function ServerManagementTable({
  ...
  const getCPUBars = (percentage: number, status: Server["status"]) => {
    ...
    const getBarColor = (index: number) => {
      if (index >= filledBars) {
        return isDark ? "border border-[#232323] bg-[#181818]" : "border border-[#eadfce] bg-[#f8f2e7]";
      }
  ...
  const getStatusGradient = (status: Server["status"]) => {
    switch (status) {
      case "active":
        return isDark ? "from-[#77b58b]/10 to-transparent" : "from-[#77b58b]/7 to-transparent";
      case "paused": 
        return isDark ? "from-[#ec5c13]/12 to-transparent" : "from-[#ec5c13]/8 to-transparent";
      case "inactive":
        return isDark ? "from-[#cf7c72]/10 to-transparent" : "from-[#cf7c72]/7 to-transparent";
    }
  };
        ...
        <AnimatePresence>
          {selectedServer && (
            <motion.div
              ...
              className={`absolute inset-0 z-10 flex flex-col overflow-hidden rounded-[18px] backdrop-blur-sm ${isDark ? "bg-[#080808]/62" : "bg-[#f7f3ea]/70"}`}
            >
              {/* Header with Actions */}
              <div className={`relative flex items-center justify-between border-b bg-gradient-to-r p-4 ${palette.divider} ${isDark ? "from-[#181818]" : "from-[#fbf7ef]"} to-transparent`}>
                <div className="flex items-center gap-4">
