...
export function LeadsTable({
  ...
  const gridTemplateColumns = "260px 112px 124px 100px 118px 132px minmax(160px,1fr)";
  ...
  return (
    <div className={`relative mx-auto w-full max-w-[1180px] ${className}`}>
      ...
      <div className={`overflow-hidden rounded-[18px] border ${palette.container}`}>
        ...
        {/* Table Headers */}
        <div
          className={`grid w-full border-b px-5 py-3 text-xs font-medium uppercase tracking-wide ${palette.header}`}
        ...
        >
              ...
              <div>Interest</div>
              <div>Probability</div>
              <div className="flex min-w-0 cursor-pointer items-center gap-2 pr-3" onClick={handleSort}>
                Last Action
                <ChevronDown className={`h-4 w-4 shrink-0 transition-transform ${sortOrder === "asc" ? "rotate-180" : ""}`} />
              </div>
            </div>
        ...
        >
          {leads.map((lead, index) => (
            <motion.div key={lead.id} variants={shouldAnimate ? rowVariants : {}}>
              <div
                className={`group relative grid w-full cursor-pointer px-5 py-2 transition-colors ${
                    ...
                    <AnimatePresence mode="wait">
                      {hoveredAction === lead.id ? (
                        ...
                        <motion.span
                          ...
                          className={`truncate text-xs ${palette.secondaryText}`}
                        >
                          {lead.lastAction}
                        </motion.span>
                      )}
