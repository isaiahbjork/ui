...
type BjorkCalendarVariant = "outline" | "base";

type BjorkCalendarProps = React.ComponentProps<typeof Calendar> & {
  variant?: BjorkCalendarVariant;
};

const calendarVariants: Record<BjorkCalendarVariant, string> = {
  outline:
    "border-[color:var(--bjork-border)] bg-[var(--bjork-bg)] shadow-[var(--bjork-shadow-soft)]",
  base:
...
export function BjorkCalendar({
  ...
  return (
    <Calendar
      className={cn(
        "rounded-[18px] border p-3 text-[color:var(--bjork-text)]",
